...
export const useGetSummary = ()=>{
  ...
  const query = useQuery({
    // TODO: Check if params are needed int the query key
    queryKey:["summary",{from,to,accountId}],
    queryFn: async()=>{
      const response = await client.api.summary.$get({query:{from,to,accountId}});
      if(!response.ok){
        throw new Error("ERROR_GET_SUMMARY: Failed to fetch summary");
      }
      const {data} = await response.json();

      return {
        ...data,
        income: convertAmountFromMiliunits(data.incomeAount),
